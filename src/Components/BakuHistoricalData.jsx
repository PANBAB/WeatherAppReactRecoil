...
import {
  fetchBakuHistoricalData,
  selectBakuHistoricalData,
} from "../redux/BakuHistoricalWeatherSlice.jsx";
import "./Loader.css";

const BakuHistoricalData = () => {
  const dispatch = useDispatch();
  const bakuHistoricalData = useSelector(selectBakuHistoricalData);
  const [expandedIndex, setExpandedIndex] = React.useState(null);

  useEffect(() => {
    if (!bakuHistoricalData) {
      dispatch(fetchBakuHistoricalData());
    }
  }, [dispatch, bakuHistoricalData]);

  const fetchHistoricalData = () => {
    dispatch(fetchBakuHistoricalData());
  };

  if (!bakuHistoricalData || !bakuHistoricalData.daily) {
    return (
      <div>
  ...
  const { daily } = bakuHistoricalData;
  ...
  return (
    <div>
      <Button variant="outlined">
        <Link to="/baku"> Back to Baku exp</Link>
...
export default BakuHistoricalData;
