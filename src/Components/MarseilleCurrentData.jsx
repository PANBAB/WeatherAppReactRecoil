...
} from "../redux/MarseilleWeatherSlice";

const MarseilleCurrentData = () => {
  const dispatch = useDispatch();
  const MarseilleweatherData = useSelector(selectMarseilleWeatherData);
  const [selectedHour, setSelectedHour] = useState(
    new Date().getHours().toString()
  );

  useEffect(() => {
    if (!MarseilleweatherData) {
  ...
  const { daily, hourly } = MarseilleweatherData;
...
export default MarseilleCurrentData;
