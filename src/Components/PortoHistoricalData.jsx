...
import {
  fetchPortoHistoricalData,
  selectPortoHistoricalData,
} from "../redux/PortoHistoricalWeatherSlice";
import "./Loader.css";

const PortoHistoricalData = () => {
  const dispatch = useDispatch();
  const portoHistoricalData = useSelector(selectPortoHistoricalData);
  const [expandedIndex, setExpandedIndex] = React.useState(null);

  useEffect(() => {
    if (!portoHistoricalData) {
      dispatch(fetchPortoHistoricalData());
    }
  }, [dispatch, portoHistoricalData]);

  const fetchHistoricalData = () => {
    dispatch(fetchPortoHistoricalData());
  };

  if (!portoHistoricalData || !portoHistoricalData.daily) {
    return (
      <div>
  ...
  const { daily } = portoHistoricalData;
  ...
  return (
    <div>
      <Button variant="outlined">
        <Link to="/porto">Porto current weather</Link>
...
export default PortoHistoricalData;
