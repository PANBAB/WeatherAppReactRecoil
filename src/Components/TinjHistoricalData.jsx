...
import {
  fetchTinjHistoricalData,
  selectTinjHistoricalData,
} from "../redux/TinjHistoricalWeatherSlice";
import "./Loader.css";

const TinjHistoricalData = () => {
  const dispatch = useDispatch();
  const tinjHistoricalData = useSelector(selectTinjHistoricalData);
  const [expandedIndex, setExpandedIndex] = React.useState(null);

  useEffect(() => {
    if (!tinjHistoricalData) {
      dispatch(fetchTinjHistoricalData());
    }
  }, [dispatch, tinjHistoricalData]);

  const fetchHistoricalData = () => {
    dispatch(fetchTinjHistoricalData());
  };

  if (!tinjHistoricalData || !tinjHistoricalData.daily) {
    return (
      <div>
  ...
  const { daily } = tinjHistoricalData;
  ...
  return (
    <div>
      <Button variant="outlined">
        <Link to="/tinj">Tinj current weather</Link>
...
export default TinjHistoricalData;
